...
const t = (key, params) => window.MiaoqieI18n.t(key, params);
const localeTag = () => (window.MiaoqieI18n.getLocale() === 'zh' ? 'zh-CN' : 'en-US');
const listSep = () => (window.MiaoqieI18n.getLocale() === 'zh' ? '，' : ', ');
...
function setAccountsToolbarBusy(busy) {
  ['#btn-export-accounts', '#btn-import-file', '#btn-import-trae', '#btn-refresh-usage'].forEach((sel) => {
    const el = $(sel);
...
function updateOverview(accounts) {
  ...
  // 更新顶部 Header 统计
  const headerCount = document.getElementById('header-account-count');
  const headerCurrent = document.getElementById('header-current-account');
  if (headerCount) headerCount.textContent = allAccounts.length;
  if (headerCurrent) {
    const cur = allAccounts.find(a => a.isCurrent);
    headerCurrent.textContent = cur ? (cur.name || cur.email || cur.userId || '—') : '—';
  }
  ...
  return `
    <div class="account-card ${a.isCurrent ? 'current' : ''} ${a.tokenExpired ? 'token-expired' : ''}" data-id="${a.id}">
      ...
      <div class="account-actions">
        ...
        <button class="btn btn-ghost btn-action btn-danger-text btn-remove" data-id="${a.id}">${escapeHtml(t('common.delete'))}</button>
      </div>
    </div>`;
}
...
async function handleSwitch(id, skipCheck = false) {
  ...
  // If auto-open failed, prompt user to configure Trae CN path
  if (result.autoOpenFailed) {
    const msg = t('toast.autoOpenFailed', { reason: result.autoOpenFailed });
    if (confirm(msg + '\n\n' + t('toast.goSettings'))) {
      switchPage('settings');
    }
  }
...
function applyTheme(theme) {
  ...
  try { localStorage.setItem('miaoqie-theme', mode); } catch { /* */ }
...
async function loadSettings() {
  ...
  const lang = appSettings.language || window.MiaoqieI18n.detectLocale();
  window.MiaoqieI18n.initI18n(lang);
...
document.body.addEventListener('click', async (e) => {
  if (e.target.dataset.action === 'open-add' || e.target.dataset.action === 'import-from-trae') {
    try {
      const result = await unwrap(api.importFromTrae());
      const label = result.account.email || result.account.name || result.account.tenantId;
      showToast(result.created ? t('toast.importedNew', { email: label }) : t('toast.importedUpdated', { email: label }));
  ...
  const captureBtn = e.target.closest('.btn-capture-auth');
  if (captureBtn) {
    captureBtn.disabled = true;
    try {
      const result = await unwrap(api.captureAuth(captureBtn.dataset.id));
      showToast(t('toast.authCaptured', { count: result.captured }));
      await refreshAccounts();
    } catch (err) {
      showToast(err.message, true);
    } finally {
...
function initAccountsSwitchTip() {
  ...
  if (!tip) return;
  try {
    if (localStorage.getItem('miaoqie-switch-tip-dismissed') === '1') {
...
$('#btn-dismiss-switch-tip')?.addEventListener('click', () => {
  ...
  try { localStorage.setItem('miaoqie-switch-tip-dismissed', '1'); } catch { /* */ }
});
...
$('#btn-import-trae').addEventListener('click', async () => {
  ...
  try {
    ...
    const label = result.account.email || result.account.name || result.account.tenantId;
    if (result.created) {
      showToast(t('toast.importedNew', { email: label }));
    } else {
      showToast(t('toast.importedUpdated', { email: label }));
    }
    await refreshAccounts({ syncMessage: t('sync.importing') });
  } catch (err) { showToast(err.message, true); }
  finally {
...
$('#btn-browser-login').addEventListener('click', async () => {
  ...
  status.textContent = '正在打开登录窗口...';
  try {
    await unwrap(api.startBrowserLogin());
    status.textContent = '登录窗口已打开，请在弹窗中完成登录...';
  } catch (err) {
...
api.onBrowserLoginSuccess(async (data) => {
  const wasRelogin = !!reloginAccountId;
  resetBrowserLoginState();
  if (addDialog.open) addDialog.close();
  showToast(wasRelogin ? t('toast.tokenUpdated') : t('toast.loginSuccess', { email: data.userInfo?.email || t('toast.newAccount') }));
  reloginAccountId = null;
...
$('#setting-language').addEventListener('change', async (e) => {
  ...
  window.MiaoqieI18n.setLocale(lang);
...
window.addEventListener('miaoqie:localechange', () => {
  applyTheme(appSettings.theme);
  updateAutoRefreshStatus();
  refreshSettingsLabels();
  setDisclaimerMode(disclaimerRequired);
...
async function runStartupScan() {
  const scanDialog = document.getElementById('startup-scan-dialog');
  const scanStatus = document.getElementById('startup-scan-status');
  const footerNotFound = document.getElementById('startup-scan-footer-notfound');

  const scanSteps = [
    { text: '正在扫描注册表…', delay: 0 },
    { text: '正在检查开始菜单…', delay: 600 },
    { text: '正在检查常见安装位置…', delay: 1200 },
    { text: '正在扫描其他磁盘…', delay: 1800 },
  ];
  ...
  // Animate status text through scan steps
  let scanStepIndex = 0;
  const stepTimer = setInterval(() => {
    if (scanStepIndex < scanSteps.length && scanStatus) {
      scanStatus.textContent = scanSteps[scanStepIndex].text;
      scanStepIndex++;
    }
  }, 700);

  try {
    const result = await api.startupScanTrae();

    clearInterval(stepTimer);

    if (result.found) {
      cachedTraePath = result.path;
      traePathEl.textContent = result.path;
      if (scanStatus) {
        scanStatus.textContent = result.alreadyConfigured
          ? 'Trae CN 路径已配置 ✓'
          : '已找到 Trae CN ✓';
      }
      // Auto-close after brief display
      setTimeout(() => {
        if (scanDialog && scanDialog.open) scanDialog.close();
      }, 800);
    } else {
      // Not found — switch to manual select UI
      const spinner = scanDialog?.querySelector('.loading-spinner-lg');
      const title = scanDialog?.querySelector('h2');
      if (spinner) spinner.style.display = 'none';
      if (title) title.textContent = '未找到 Trae CN';
      if (scanStatus) scanStatus.textContent = '程序中未保存 Trae CN 路径，请手动选择';
      if (footerNotFound) footerNotFound.classList.remove('hidden');
    }
  } catch (err) {
    clearInterval(stepTimer);
    if (scanStatus) scanStatus.textContent = '扫描失败: ' + (err.message || '未知错误');
    if (footerNotFound) footerNotFound.classList.remove('hidden');
  }
}

// ── Startup scan dialog buttons ──

document.addEventListener('click', async (e) => {
  // Manual select from startup scan dialog
  if (e.target.id === 'btn-startup-manual-select') {
    e.preventDefault();
    const scanDialog = document.getElementById('startup-scan-dialog');
    try {
      const p = await unwrap(api.pickTraePath());
      if (p) {
        cachedTraePath = p;
        traePathEl.textContent = p;
        showToast(t('toast.pathSaved'));
        if (scanDialog && scanDialog.open) scanDialog.close();
      }
    } catch (err) {
      showToast(err.message, true);
  ...
  // Skip from startup scan dialog
  if (e.target.id === 'btn-startup-skip') {
    e.preventDefault();
    const scanDialog = document.getElementById('startup-scan-dialog');
    if (scanDialog && scanDialog.open) scanDialog.close();
  }
...
(async () => {
  try {
    await window.MiaoqieI18n.ready;
    initAccountsSwitchTip();
    await loadSettings();
    if (!appSettings.language) await saveSettings({ language: window.MiaoqieI18n.getLocale() });
    await ensureDisclaimerAccepted();
    await loadTraePath();
    await refreshAccounts();

    // 启动后检查 Trae CN 路径，未配置时弹出扫描窗口
    const p = await unwrap(api.getTraePath());
    if (!p) {
      // 页面已经渲染完毕，此时弹出 Loading 弹窗进行扫描
      runStartupScan();
    }

    // 无边框窗口：顶部 Header 关闭按钮
    const headerCloseBtn = document.getElementById('header-close-btn');
    if (headerCloseBtn) {
      headerCloseBtn.addEventListener('click', () => window.traeAccounts.quitApp());
    }
  } catch (err) {
    initialLoading = false;
    renderUI();

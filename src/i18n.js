(function () {
  const STORAGE_KEY = 'miaoqie-lang';
  
  // 迁移旧语言设置
  try {
    const oldLang = localStorage.getItem('traeswitch-cn-lang');
    if (oldLang) {
      localStorage.setItem('miaoqie-lang', oldLang);
      localStorage.removeItem('traeswitch-cn-lang');
    }
  } catch { /* ignore */ }
  ...
  function applyDomI18n(root = document) {
    const textPairs = [
      ...
      ['#overview-empty [data-action="import-from-trae"]', 'accounts.importFromTrae'],
  ...
  function setLocale(next, { persist = true } = {}) {
    ...
    applyDomI18n();
    window.dispatchEvent(new CustomEvent('miaoqie:localechange', { detail: { locale: next } }));
  }

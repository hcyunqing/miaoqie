// Fix Windows console encoding to prevent garbled Chinese output.
// Node.js uses UTF-8 internally, but Windows cmd.exe defaults to GBK (CP936).
// execSync('chcp 65001') only affects the child cmd.exe, NOT the current process.
// Instead, configure stdout/stderr to always output UTF-8 bytes.
...
const { switchTraeAccount, scanTraePath, captureICloudAuthInfo, writeAuthToStorageJson } = require('./trae-switcher');

// Safe console.log wrapper (handles Windows encoding issues)
...
function createWindow() {
  ...
  mainWindow.setBackgroundColor('#00000000'); // 强制窗口背景透明，防止 DWM 用系统颜色绘制标题栏区域
  mainWindow.loadFile(path.join(__dirname, '../src/index.html'));
  mainWindow.webContents.on('did-finish-load', setupStorageWatcher);
  mainWindow.on('focus', notifyTraeAccountChanged);

  // 修复 Windows 无边框窗口从任务栏恢复/激活后出现标题栏的 DWM bug
  // DWM 在窗口 z-order 变化时可能错误重绘非客户区，短暂改变尺寸强制 DWM 重算
  if (process.platform === 'win32') {
    let fixFrameTimer = null;
    const fixFrame = () => {
      if (fixFrameTimer) clearTimeout(fixFrameTimer);
      fixFrameTimer = setTimeout(() => {
    ...
    mainWindow.on('restore', fixFrame);
    mainWindow.on('show', fixFrame);
    mainWindow.on('focus', () => {
      // focus 在 restore 之后触发，延迟确保 DWM 完成绘制后修复
      if (fixFrameTimer) clearTimeout(fixFrameTimer);
      fixFrameTimer = setTimeout(() => {
        if (!mainWindow || mainWindow.isDestroyed() || mainWindow.isMinimized()) return;
        if (mainWindow.getBounds().y === -32000) return; // minimized position
        const [w, h] = mainWindow.getSize();
        mainWindow.setSize(w, h + 1);
        setTimeout(() => {
          if (mainWindow && !mainWindow.isDestroyed()) mainWindow.setSize(w, h);
...
async function performSwitch(id) {
  ...
  // If the cookie-based API refreshed the token, persist it
  if (result.refreshedToken) {
    accountStore.persistAccountToken(id, result.refreshedToken, {
      tokenExp: result.refreshedTokenExp,
      tokenExpiredAt: result.refreshedExpiredAt,
    });
  }
...
ipcMain.handle('accounts:import-from-trae', wrap(async () => {
  // 1. Capture encrypted iCubeAuthInfo blobs from Trae CN's storage.json
  const authBlob = captureICloudAuthInfo();
  if (!authBlob) throw new Error('未在 Trae CN 中检测到登录态，请先在 Trae CN 中手动登录');

  // 2. Try to get user metadata (optional: from trae-jwt-token or cookies)
  let metadata = {};
  try {
    ...
    // Token file doesn't exist — that's fine, we'll import with minimal info
  }

  // 3. Import (create or update) account with captured authBlob
  const result = accountStore.importAccountWithAuthBlob(authBlob, metadata);
  return {
    ...result,
    source: 'trae-cn-storage.json',
  };
}));
...
// Capture the encrypted iCubeAuthInfo blob from Trae CN's storage.json
// Call this after the user has manually logged in to Trae CN.
ipcMain.handle('accounts:capture-auth', wrap(({ id }) => {
  if (!id) throw new Error('请指定账号 ID');
  const authBlob = captureICloudAuthInfo();
  if (!authBlob) throw new Error('未能捕获到登录凭证，请确认 Trae CN 当前已登录');
  return accountStore.captureAccountAuthBlob(id, authBlob);
}));
...
ipcMain.handle('settings:scan-trae-path', wrap(() => {
  const found = scanTraePath();
  accountStore.setTraePath(found);
  return found;
}));

// 启动后自动扫描 Trae CN（页面加载完成后从渲染进程调用）
ipcMain.handle('settings:startup-scan-trae', wrap(async () => {
  // Check if path already configured
  const existing = accountStore.getTraePath();
  if (existing && fs.existsSync(existing)) {
    return { found: true, path: existing, alreadyConfigured: true };
  }
  ...
  if (found) {
    accountStore.setTraePath(found);
    return { found: true, path: found, alreadyConfigured: false };
  }

  return { found: false, path: null, alreadyConfigured: false };
}));
...
ipcMain.handle('accounts:start-browser-login', wrap(async ({ accountId } = {}) => {
  ...
  startBrowserLogin({
    ...
    onSuccess: async (cookieString, userInfo, tokenInfo = {}) => {
      // 通知渲染进程登录成功
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('browser-login:success', { cookieString, userInfo });
      }
      ...
      const extras = {
        ...
        userId: tokenInfo.tokenUserId || userInfo.userId || '',
        tenantId: tokenInfo.tokenTenantId || userInfo.tenantId || '',
        token: tokenInfo.token || '',
        expiredAt: tokenInfo.expiredAt || '',
      };
      if (reloginAccountId) {
        const account = await accountStore.updateAccountCookie(reloginAccountId, cookieString, extras);
        reloginAccountId = null;
        trayApi?.rebuild();
        return account;
      }
      const account = await accountStore.addAccountByCookie(cookieString, userInfo, extras);
      trayApi?.rebuild();
      return account;
    },
    onFailed: (err) => {
      safeLog(`[main] 浏览器登录失败: ${err.message}`);
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('browser-login:failed', { error: err.message });
      }
    },
    onCancelled: () => {
      safeLog('[main] 用户取消了浏览器登录');
      reloginAccountId = null;
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('browser-login:cancelled');
      }
    },
    onStatusChange: (status) => {
      safeLog(`[main] 浏览器登录状态: ${status}`);
      if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('browser-login:status', { status });
      }
    },
  });

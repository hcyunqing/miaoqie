...
async function killProcessesWin(log, cancelled) {
  const images = ['Trae CN.exe', 'trae-cn.exe', 'Trae CN Helper.exe', 'Trae CN Helper (GPU).exe'];
...
async function resetPreferencesMac(log, cancelled) {
  ...
  await runCmdBatch(
    [
      'defaults delete com.trae.cn 2>/dev/null || true',
...
async function resetPreferencesWin(log, cancelled) {
  log('重置偏好设置...');
  const keys = ['HKCU\\Software\\com.trae.cn'];
...
async function resetMachineIdMac(log, machineIdPath, cancelled, removePathFn) {
  // Trae CN does not use a separate machineId file on any platform.
  log('重置设备标识 (machineId)...');
  log('  Trae CN 不使用独立 machineId 文件，跳过');
}
...
async function cleanCredentialsMac(log, cancelled) {
  log('清理 Keychain 中的 Trae CN 条目...');
  await runCmdBatch(
    [
      "security delete-generic-password -s 'trae.cn' 2>/dev/null || true",
      "security delete-generic-password -s 'com.trae.cn' 2>/dev/null || true",
    ],
    log,
    cancelled
  );
...
async function cleanCredentialsWin(log, cancelled) {
  log('清理 Windows 凭据管理器中的 Trae CN 条目...');
  const script = `
    $targets = @('trae.cn', 'com.trae.cn', 'Trae CN')

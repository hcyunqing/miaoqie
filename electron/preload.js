...
contextBridge.exposeInMainWorld('traeAccounts', {
  ...
  captureAuth: (id) => ipcRenderer.invoke('accounts:capture-auth', { id }),
  ...
  // 浏览器登录状态监听
  onBrowserLoginSuccess: (cb) => {
    const handler = (_e, data) => cb(data);
    ipcRenderer.on('browser-login:success', handler);
    return () => ipcRenderer.removeListener('browser-login:success', handler);
  },
  onBrowserLoginFailed: (cb) => {
    const handler = (_e, data) => cb(data);
    ipcRenderer.on('browser-login:failed', handler);
    return () => ipcRenderer.removeListener('browser-login:failed', handler);
  },
  onBrowserLoginCancelled: (cb) => {
    const handler = () => cb();
    ipcRenderer.on('browser-login:cancelled', handler);
    return () => ipcRenderer.removeListener('browser-login:cancelled', handler);
  },
  onBrowserLoginStatus: (cb) => {
    const handler = (_e, data) => cb(data);
    ipcRenderer.on('browser-login:status', handler);
    return () => ipcRenderer.removeListener('browser-login:status', handler);
  },
  ...
  startupScanTrae: () => ipcRenderer.invoke('settings:startup-scan-trae'),
